...
def makeInverseEnergy(eBins,eMin,eMax,eRes):
    ...
    print("Energy step limit --")
    print("    Grid resolution:",eRes,
          " fraction:",targetFraction)
    print("    Minimum fraction:",minFraction)
    print("    Maximum fraction:",maxFraction)
    print("    Fraction used:",fractionLimit)
...
parser.add_argument("-E","--energy-resolution",type=float,default=-1.0,
                    help="The target limit on ratio between energy steps")
args = parser.parse_args()

# Find the energy resolution for the binning.  This makes sure that
# the energy step is never more than this "resolution".  If the energy
# resolution is uninitialized, the a default value is calculated based
# on the total number of grid points being used.  In practice, this
# changes the grid spacing at the highest energies.  NOTE: The default
# resolutions are "reasonable", but not particularly special.
energyResolution = args.energy_resolution
if (energyResolution < 0):
    dLog = math.log(args.max_energy) - math.log(args.min_energy)
    dLog = dLog / (0.2*args.energy_bins)
    energyResolution = math.exp(dLog) - 1.0
    print("Using default maximu energy grid resolution: ", energyResolution)
...
file = ("grid_E%d.root"  % (len(energies)-1))

output = ROOT.TFile(file,"recreate")
